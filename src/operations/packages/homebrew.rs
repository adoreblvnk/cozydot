...
pub(crate) fn install(host: &Host) -> Result<()> {
    if find_executable(host)?.is_some() {
        return Ok(());
    }
...
pub(crate) fn install_packages(host: &Host, formulae: &[String], casks: &[String]) -> Result<()> {
    let brew = find_executable(host)?.context(HOMEBREW_UNAVAILABLE)?;
...
pub(crate) fn executable_path(host: &Host, formula: &str, executable: &str) -> Result<String> {
    let brew = find_executable(host)?.context(HOMEBREW_UNAVAILABLE)?;
...
fn find_executable(host: &Host) -> Result<Option<String>> {
