use crate::{config::HttpsUrl, operations, platform::Architecture, runner::Step};
use std::{
    fs,
    io::Write,
...
impl Host {
    fn new() -> Self {
        ...
        let cozydot = std::env::var_os("CARGO_BIN_EXE_cozydot")
            .map(PathBuf::from)
            .unwrap_or_else(|| {
                std::env::current_exe()
                    .unwrap()
        ...
        symlink(cozydot, bin.join("cozydot")).unwrap();
        Self {
  ...
  *) exit 45 ;;
    ...
    fn run_with_path(&self, step: &Step, path: String) -> std::process::Output {
        ...
        let result = operations::execute_with_docker_lock_for_test(
            step.operation().expect("lowered workflow step"),
            &env,
            &self.root.join("run/cozydot/docker-daemon.lock"),
        );
...
#[test]
fn binary_no_state_does_not_adopt_path_command() {
    let host = Host::new();
...
fn binary_rejects_unsafe_data_and_command_roots_before_network() {
    ...
    for (data, bin) in [
        ...
        assert!(host
            .execute_operation_with_xdg_roots(
                operation.operation().expect("lowered workflow step"),
                &data,
                &bin,
            )
            .is_err());
