...
- Pass 004c also adds the canonical per-user Flathub operation set: strict fixed-identity remote inspection and idempotent ensure, one strict installed-app inspection followed by one ordered missing-ref install batch, and one update targeting configured application IDs while retaining required runtime/related-ref updates and accepting upstream-declared end-of-life replacements. Remote identity, URL, user scope, and flags are not operation inputs.
