...
- Managed-source correction now restricts `managed` to pure Ubuntu, pure Debian, and Kali; all derivatives and Tails require `preserve`. Platform detection keeps distribution and base codenames separately, classifies Linux Mint versus LMDE from `ID_LIKE`, and no longer treats Deepin as Ubuntu. Release-aware typed policy generation validates current Ubuntu/Debian/Kali architecture, component, keyring, suite, and archive routing before operation construction. The operation semantically reconciles both `.list` and deb822 `.sources`, preserves source-package and unrelated/vendor state, makes content-addressed mode-`0600` backups before any rewrite, publishes individual files durably with the owned source last, and documents/retries partial multi-file migration without claiming transaction atomicity.
- Managed-source verification initially exposed a stale Debian fixture carrying Ubuntu's `noble` codename, loss of indexed component diagnostics, stale Deepin-as-Ubuntu planner expectations, and one allocation-heavy digest formatter rejected by strict Clippy. Those failures were corrected rather than discarded. Adversarial review then added tab-separated one-line parsing, preservation of non-HTTP local/transport sources, fixed keyring and source-directory no-follow preflights, and an absent/symlink precondition before first owned-file publication. Final acceptance passed 15 repeated managed-operation suites, 15 parser/reconciler suites, 10 complete config and planner suites each, locked formatting and strict Clippy, and three serialized locked full suites.
...
- Pass 004e adds operation-owned typed existing-product integration foundations for Docker group membership, VirtualBox group membership, Docker's daemon-wide local log driver, and ordered VS Code extension sets. Every operation defensively validates its own inputs, probes the canonical existing product CLI, strictly parses UTF-8 state, applies only missing state through fixed argv without a shell or package manager, and verifies mutations. Group operations resolve the process effective UID through one exact NSS passwd record, compare numeric GIDs from exact `getent`/`id` records, create only their fixed system group when absent, never run `newgrp`, and leave new supplementary membership to logout/login. Docker preserves unrelated JSON under the fixed cooperative `/run/cozydot/docker-daemon.lock` advisory transaction lock in a root-owned runtime directory, publishes `/etc/docker/daemon.json` through the accepted root-owned mode-`0644` atomic helper with file and parent-directory sync, and performs no daemon restart or reload; active behavior changes at Docker's normal reload/restart boundary. VS Code canonicalizes valid two-component IDs to lowercase, installs canonical missing IDs in configuration order, and requeries once.
...
- Pass 004e final acceptance independently confirmed the descriptor-held Docker lock and retry-durable parent sync. The original Fix 2 run first exposed and then corrected fake-harness assumptions in its focused Docker and repository tests; those failures remain part of the acceptance record rather than being superseded by later green output. Takeover review additionally found that ordinary lock-file open and setup commands could follow a symlink across check/use boundaries. The accepted correction creates an absent lock with fixed non-clobbering `cp --no-clobber --no-target-directory -- /dev/null`, rejects non-regular state before no-dereference ownership repair, and opens the fixed path through Rustix with `O_NOFOLLOW | O_CLOEXEC` before taking the blocking exclusive flock. Regressions prove a setup-path symlink and referent remain unchanged, a test-only open-path symlink fails before Docker config reads, two operations serialize on one preserved inode, all exercised failures release the descriptor, and a failed post-rename parent sync succeeds on the next semantic no-op without republishing. Acceptance passed 25 serialized Docker-focused runs, 25 repository-focused runs, locked formatting and strict Clippy, and three serialized locked full suites. No helper process or public lock-path configuration remains.
- Pass 004f adds the remaining typed executable foundations and one complete schema-v1 lowering boundary without switching production config loading. Rust toolchains use target-qualified Rustup names, strict verbose release/host state, fixed minimal installs, explicit defaults, and moving-channel-only refreshes. Official Go installs resolve stable metadata including partial exact selectors, verify published SHA-256 values, preflight archive paths, verify staged and installed version/architecture, and preserve exact selectors. FNM Node installs use fixed direct argv and owned selector aliases instead of shell evaluation; ensure mode preserves the selected moving release while update mode resolves it explicitly. UV Python requires managed interpreter state and verifies the configured numeric series after installation.
- Pass 004f also adds user-local Nerd Font publication with archive preflight, destination-filesystem staging, extracted-tree validation, Fontconfig refresh, and exact family postconditions; backup-before-Stow dotfiles with deterministic conflict discovery, unique timestamped state backups preserving target-relative paths, package-local abort before Stow on backup failure, and resolved-link postconditions; and typed GNOME/Cinnamon theme, terminal, idle, GNOME extension, dock, and rounded-corner operations with exact state requery. GNOME extension batches emit one documented login-boundary notice when newly installed extensions are not yet registered.
- System foundations now cover effective-user membership in `sudo`, enabled/disabled unattended-upgrade package, service, and periodic-file state, and enabled/disabled Ubuntu Snap package, service, data, and fixed no-Snap pin state. Standalone Rustup, FNM, and UV bootstraps allow inferred prerequisites to precede consumers without manager selection in YAML.
- `planner::lower_v1` exhaustively lowers every `PlannedAction` to workflow operations. Complete-plan tests prove inferred APT bootstrap precedes sources and consumers, manager bootstraps precede tool/package use, repository key/source publication is derived once, Flathub is ensured once, the explicit post-source APT refresh occurs once, all granular updates retain configured scope, desktop mismatch remains a planner-only skip, and no command, shell, conditional, or legacy language/package operation appears in schema-v1 output. Production remains on the legacy loader until the preset/init/apply migration is changed atomically in milestone 5.
- Pass 005a migrates all four bundled presets and the embedded default to strict schema v1, switches production `apply` to `ConfigV1` validation, typed planning, and complete workflow lowering, and removes the old post-apply YAML mutation. Preset tests plan and lower every preset on Ubuntu and Debian, assert CLI/VM scope, preserve one post-source metadata refresh, and prohibit implicit Docker or VirtualBox installation/configuration. An offline `init` followed by dry-run `apply` completes against the embedded default. README and rewrite documentation now describe the schema-v1 execution model. Legacy-only parser/planner code and superseded tests remain an explicit cleanup item before release acceptance; they are not used by production apply.
- Pass 005b removes the legacy tagged-YAML parser, string-path planner, obsolete appimaged/download/compatibility operation modules, legacy operation variants, their ETXTBSY-only execution helper, stale fixtures, and all 17 superseded ignored tests. `config` and `planner` now expose only schema-v1 modules, repository-wide Rust searches find no legacy operation names or compatibility callers, and the complete suite runs with zero ignored tests.
- Pass 005b acceptance added direct retry coverage for Rustup, FNM, and UV bootstraps plus enabled/disabled unattended-upgrade and Snap state. The third pre-fix repeated full suite exposed a timing race in the Docker lock-release test probe; the probe now waits up to 500 ms for child-process cleanup while still failing a persistent held lock. That focused regression passed 25 repetitions, followed by three serialized complete locked suites with 226 tests and zero ignored tests. Locked strict Clippy, formatting, scoped whitespace checks, Bash syntax, and the release build pass. Two package runs produced the same one-file archive digest `b45ef66a701b55f3534e7bbb018f155383b394c0aedff39fd069bd4e93c24bfa`; its checksum verifies when checked from the artifact directory. The first checksum invocation from the repository root failed only because the checksum record intentionally uses the archive basename, and is retained in this acceptance record rather than hidden.
