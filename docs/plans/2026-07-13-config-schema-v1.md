...
- Normalize required machine aliases and translate Debian, Go, Rust, and release-asset names internally.
- Migrate current architecture callers without changing embedded YAML.

TDD gate:

- Table tests cover every required normalization alias and all ecosystem translations.
...
- Normalize omission, `null`, `false`, and empty collections at the typed boundary without creating alternate public forms.
- Validate names, URLs, versions, duplicate entries, repository URL selection, direct asset maps, and cross-field requirements.
...
- Add one negative test for each unknown field, wrong type, enum, URL, duplicate, missing required child, schema version, and native-asset failure class.
- Assert omission/`null`/`false`/empty disabling for every optional section category.
...
- Plan in contract order: system preparation, removals, repositories/APT, Flatpak, tools, Cargo/NPM/direct packages, fonts, dotfiles, integrations, desktop, then enabled updates.
- Infer prerequisites from enabled features and coalesce APT metadata refresh/install work.
- Generate APT source entries from typed fields, the selected distro URL, inferred key path, and `Architecture::debian()`.
- Select exact direct-package assets by canonical architecture and fixed format handler.
...
- Repository tests cover distro URL precedence, default fallback, signed-by output, and native architecture.
- Direct-package tests cover all canonical architecture keys, absent-native-asset failure, and exact filename selection.
- Existing operation behavior tests are migrated rather than duplicated.

Exit criteria:

- Planner has no dotted string lookups, YAML tags, raw repository lines, asset globs, or public interpolation.
...
- Implement exact GitHub asset lookup and fixed `deb`/`appimage` installation.
...
- Fake-host tests prove first install, idempotent second apply, requested update, malformed metadata, checksum failure, and interrupted download behavior.
...
- Keep all software under `packages`; remove exposed prerequisite lists and unsupported legacy controls.
...
- Parse and plan every preset on each supported canonical architecture using deterministic platform fixtures.
...
- GNOME VM: terminal, configured extensions, fixed settings, dock, rounded corners, fonts, and Stow behavior match the preset after relogin where required.
- APT repository VM: distro-specific URL selection, default fallback, inferred signed-by path, and native Debian architecture are correct in generated files.
...
- Direct-package VM: the native canonical asset is selected exactly; a fixture missing that architecture fails before download.
- Update VM: each `updates` leaf changes only its manager/package set; all omitted, `null`, `false`, and empty controls produce no update step.
