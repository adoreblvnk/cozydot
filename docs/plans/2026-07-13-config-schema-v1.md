...
- Translate `updates.apt` over the system APT package set according to its scalar policy; it consumes the earlier explicit refresh and lowering must not refresh again. Update only configured Flatpak refs plus required runtimes, configured Cargo/NPM names, and configured direct definitions; never update unrelated installed packages. Require declared Node for every non-empty NPM declaration and each corresponding `tools.*` declaration for tool updates, producing no update step when required non-empty lists or tool declarations are absent.
...
- Pass 004c also adds the canonical per-user Flathub operation set: fixed idempotent remote ensure, one strict installed-app inspection followed by one ordered missing-ref install batch, and one update scoped to exactly configured refs while retaining required dependency updates. Remote identity, URL, user scope, and flags are not operation inputs.
- The legacy `DownloadBinary` operation, legacy Flatpak steps, and production planner remain unchanged until the atomic integration pass. The new APT bootstrap and Flatpak operations are executable foundations only; no `Bootstrap`, `FlatpakInstall`, or `UpdateAction::Flatpak` intent is lowered yet. Remaining milestone-4 work includes lowering all `PlannedAction` variants, including the completed operation foundations, `DirectPackageIntent`, and `UpdateAction::Direct`, plus the other fixed-manager, dotfile, integration, and desktop handlers listed below.
