...
- `distro` is one of `auto`, `ubuntu`, `linuxmint`, `pop`, `zorin`, `deepin`, `debian`, `kali`, or `tails`. These canonical configured and detected IDs are lowercase. `auto` reads `/etc/os-release`, including `ID_LIKE` to distinguish Ubuntu-based Linux Mint from LMDE. Cozydot retains the distribution codename and its upstream/base codename separately when they differ. Deepin is Debian-family for applicability checks and is never normalized into Ubuntu. Omission defaults detection to `auto`.
...
- `apt.sources` is either `preserve` or `managed`. `preserve` leaves distribution-owned APT source files untouched. `managed` is supported only on pure Ubuntu, pure Debian, and Kali. Linux Mint (including LMDE), Pop!_OS, Zorin, Deepin, and Tails require `preserve`; Tails' Tor-aware security/network source model is never replaced. Omission or `null` is equivalent to `preserve`.
- Managed Ubuntu releases are `jammy`, `noble`, the currently supported `questing`, and `resolute`. Ubuntu archive routing is release-aware: amd64 uses the main and security archives; arm64 uses Ports through 25.10 and the main/security archives from 26.04; armhf and riscv64 use Ports. Managed Debian releases are `bullseye`, `bookworm`, and `trixie`; riscv64 starts at Trixie. Kali always uses only `kali-rolling` and never mixes Debian repositories; Kali riscv64 is unsupported. Other release/architecture combinations fail before publication.
- `apt.components` is a non-empty sequence selected from `main`, `contrib`, `non-free`, `non-free-firmware`, `restricted`, `universe`, and `multiverse`, without duplicates. It supplies the components for `apt.sources: managed`; omission defaults to `main` on Ubuntu and Debian and Kali's canonical `main contrib non-free non-free-firmware` set. Ubuntu accepts only its four archive components. Debian Bullseye does not accept `non-free-firmware`; Bookworm and Trixie do. Kali accepts only its canonical four components. It is invalid when sources are omitted or `preserve`.
...
Managed mode owns only `/etc/apt/sources.list.d/cozydot-base.sources`. Before publishing it, Cozydot semantically inspects active `.list` and `.sources` files, removes only matching binary entries for the selected distribution's exact official base URIs and current suites, preserves `deb-src`, disabled entries, unrelated repositories, non-official local/cloud mirrors and transports, unrelated comments, and unknown deb822 fields retained in surviving stanzas. A stanza that mixes official and unrelated URIs, uses an unexpected release/pocket, has unsafe architecture modifiers, is malformed, or is a symlink/special file fails before source mutation. Required distribution keyrings must already be nonempty regular files at their fixed package-owned paths.

Every existing file selected for rewriting is first copied byte-for-byte to a content-addressed root-owned mode-`0600` backup below `/var/lib/cozydot/apt-source-backups`, preserving its absolute path below the digest directory. Only after all backups succeed are old official binary entries removed one file at a time; the Cozydot-owned file is published last. Each replacement and parent directory is synced atomically at the individual-file boundary, but the multi-file migration is explicitly not atomic. A mid-sequence failure leaves complete backups and a safe retryable partial state; the next apply reinspects and converges it. Cozydot compares bytes again before each rewrite and requires a final semantic postcondition rather than masking concurrent changes.
...
- `flatpak` is a sequence of canonical Flatpak application IDs installed per-user from Cozydot's fixed Flathub remote. IDs contain at least three dot-separated ASCII identifier segments; each segment starts with a letter and then contains only letters, digits, or `_`. Schema v1 has no system-wide Flatpak mode or configurable remote name, URL, or installation scope.
...
- `npm` is a sequence of unversioned modern NPM package names installed with the Node version managed by FNM. Each entry is either a lowercase unscoped identifier or exactly `@scope/name`; identifier parts start with a lowercase ASCII letter or digit and then contain only lowercase ASCII letters, digits, `.`, `_`, or `-`. A non-empty NPM sequence requires `tools.node` for both installation and updates so the selected FNM environment and Node version are unambiguous.
- `direct` is a sequence of direct package definitions.

Managed distribution sources and all third-party repository sources are published before one explicit metadata refresh. That refresh is emitted only when an APT consumer is declared: an APT-backed system control, package removal, repository package group, native APT package list, or APT update. Source publication alone does not refresh metadata. Missing internal prerequisites are inferred from enabled behavior and are not added to `packages.apt` in the effective user configuration; any minimum old-source metadata setup needed to bootstrap those fixed prerequisites is separate from the post-publication refresh.
...
Each architecture value is one mapping with exactly two required children: `include`, one anchored wildcard pattern, and `exclude`, a sequence of zero or more anchored wildcard patterns. Every pattern must contain `*` or `?`, may use only those two wildcard operators, and matches an entire asset filename (`*` matches zero or more Unicode scalar values and `?` matches exactly one Unicode scalar value). Paths, character classes, malformed wildcard syntax, interpolation, and substitutions are invalid. Scalar selectors and selectors missing either canonical child are invalid.
...
- `apt` is one scalar string policy operating on the system APT package set, not only `packages.apt`. `off` is the explicit disabled value; omission or `null` is equivalent to `off` and schedules no APT update. `standard` consumes the plan's earlier post-source metadata refresh and then runs the normal upgrade. `full` consumes that same refresh and then runs full upgrade and purge-autoremove. Lowering must not add a second metadata refresh. YAML booleans, mappings, and other scalar values are invalid.
- `flatpak: true` targets only per-user application IDs declared in `packages.flatpak`. Flatpak may also update or install their required runtimes and related refs, and may migrate a targeted application to an upstream-declared end-of-life replacement ID. It never sweeps unrelated installed Flatpaks or the system-wide installation. It requires a non-empty configured Flatpak list; otherwise it produces no step.
...
The complete configuration is parsed and validated before host-changing operations begin. After platform detection, one unified platform-validation call checks explicit configured distro and desktop selections against the resolved platform, distribution family, managed APT components, repository URL selection, architecture, and native asset selectors before planning or mutation. `auto` remains a detection request and is not compared as a resolved value. Errors include the field path and reject unknown keys, wrong YAML types, unsupported enum values, malformed identifiers, non-HTTPS URLs, duplicate definitions, unsupported architectures, missing native architecture selectors, and missing selector children. Cozydot does not silently reinterpret invalid input.
