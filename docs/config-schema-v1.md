...
- `flatpak: true` updates only per-user refs declared in `packages.flatpak` and runtimes required by those refs, never unrelated installed Flatpaks or the system-wide installation. It requires a non-empty configured Flatpak list; otherwise it produces no step.
